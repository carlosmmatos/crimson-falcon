...
| ------ | ------------ | ----------- |
| [**deprovision**](WorkflowsApi.md#deprovision) | **POST** /workflows/system-definitions/deprovision/v1 | Deprovisions a system definition that was previously provisioned on the target CID |
| [**execute_1**](WorkflowsApi.md#execute_1) | **POST** /workflows/entities/execute/v1 | Executes an on-demand Workflow, the body is JSON used to trigger the execution, the response the execution ID(s) |
| [**execution_action**](WorkflowsApi.md#execution_action) | **POST** /workflows/entities/execution-actions/v1 | Allows a user to resume/retry a failed workflow execution. |
| [**execution_results**](WorkflowsApi.md#execution_results) | **GET** /workflows/entities/execution-results/v1 | Get execution result of a given execution |
| [**promote**](WorkflowsApi.md#promote) | **POST** /workflows/system-definitions/promote/v1 | Promotes a version of a system definition for a customer. The customer must already have been provisioned. This allows the caller to apply an updated template version to a specific cid and expects all parameters to be supplied. If the template supports multi-instance the customer scope definition ID must be supplied to determine which customer workflow should be updated. |
| [**provision**](WorkflowsApi.md#provision) | **POST** /workflows/system-definitions/provision/v1 | Provisions a system definition onto the target CID by using the template and provided parameters |
...
> <ClientSystemDefinitionCreateResponse> deprovision(body)
...
begin
  # Deprovisions a system definition that was previously provisioned on the target CID
  result = api_instance.deprovision(body)
  p result
...
#### Using the execution_action_with_http_info variant
...
> <Array(<ClientSystemDefinitionCreateResponse>, Integer, Hash)> provision_with_http_info(body)
